...
const searchLocation = async (query: string, provinceName?: string) => {
  try {
    const searchQuery = provinceName ? `${query}, ${provinceName}, Vietnam` : `${query}, Vietnam`;
...
export default function DistrictForm({
  ...
  const [province_id, setProvinceId] = useState(initial?.province_id || provinces[0]?.id || '');
  const [latitude, setLatitude] = useState(initial?.latitude ?? '');
  const [longitude, setLongitude] = useState(initial?.longitude ?? '');
  const [saving, setSaving] = useState(false);
  const [searching, setSearching] = useState(false);

  const selectedProvince = provinces.find(p => p.id == province_id);
  ...
  const handleSearch = async () => {
    ...
    setSearching(true);
    try {
      const result = await searchLocation(name, selectedProvince?.name);
      if (result) {
  ...
  const submit = async (e?: React.FormEvent) => {
    ...
    try {
      await onSubmit({ 
        name, 
        code, 
        province_id: parseInt(String(province_id)), 
  ...
  return (
    <div className="h-full flex flex-col">
      <form onSubmit={submit} className="space-y-6 flex-1">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Tên quận/huyện</label>
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <div className="text-sm text-blue-700">
            ...
            <ul className="mt-2 space-y-1">
              <li>• Chọn tỉnh/thành phố trước</li>
              <li>• Nhập tên quận/huyện rồi bấm "🔍 Tìm kiếm" để tự động điền tọa độ</li>
