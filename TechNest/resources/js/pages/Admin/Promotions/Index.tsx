...
export default function Index() {
  ...
  const remove = (id: number) => { if (!confirm('Xóa mã khuyến mãi?')) return; router.delete(`/admin/promotions/${id}`); };
          ...
          <table className="w-full">
            ...
            <tbody>
              {promotions.data.length === 0 ? (
                <tr><td colSpan={6} className="py-6 text-center text-gray-500">Chưa có khuyến mãi</td></tr>
              ) : promotions.data.map((p:any) => {
                const conds = p.conditions ?? [];
                const applyText = conds.length === 0 ? 'Tất cả sản phẩm' : conds.map((c:any) => `${c.condition_type}#${c.target_id}`).join(', ');
                return (
